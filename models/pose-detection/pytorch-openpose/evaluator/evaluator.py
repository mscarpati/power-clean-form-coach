...
class Evaluator():
        ...
        if len(candidate) > 0:
            ...
            if len(coordinates) > 15:
                ...
                if (abs(coordinates[4][1] - coordinates[2][1]) <= threshold or 
                    ...
                    left = util.get_angle(coordinates[8], coordinates[9], coordinates[10])
                    right = util.get_angle(coordinates[12], coordinates[13], coordinates[14])
                    if left < self.knee_angle_L or right < self.knee_angle_R:
                        ...
                        self.knee_coord_R = coordinates[14] # corresponds to 13
                        return True
    ...
    def calculate_hip(self, candidate, height):
        ...
        if len(candidate) > 0:
            ...
            if (abs(coordinates[4][1] - coordinates[8][1]) <= threshold 
                or abs(coordinates[7][1] - coordinates[11][1]) <= threshold):
    ...
    def draw(self, frame):
        ...
        if knee_color is not None:
            ...
            if knee == 'left':
                cv.circle(frame, (int(self.knee_coord_L[0]), int(self.knee_coord_L[1])), 10, knee_color, -1)
            else:
                cv.circle(frame, (int(self.knee_coord_R[0]), int(self.knee_coord_R[1])), 10, knee_color, -1)

        if hip_color is not None:
            # draw circle around hip - center point should be the hip coordinate
            if hip == 'left':
                cv.circle(frame, (int(self.hip_coord_L[0]), int(self.hip_coord_L[1])), 10, hip_color, -1)
            else:
                cv.circle(frame, (int(self.hip_coord_R[0]), int(self.hip_coord_R[1])), 10, hip_color, -1)
    ...
    def process_image(self, frame, is_single=False):
        ...
        knee = self.calculate_knee(candidate, frame.shape[0])
        hip = self.calculate_hip(candidate, frame.shape[0])
    ...
    def process_vid(self, video):
        ...
        video_out = cv.VideoWriter("res/out.mp4", fourcc, fps, (w,h), isColor=True)
